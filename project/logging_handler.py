...
def begin_cpu_watcher():
    ...
    def call_cpu_watcher():
        if not busy.read(): caller.invokeMethod(cpu_obj, 'get_cpu') 
...
#filepath
filepath = os.path.join(g.main_dir.read(), 'logs', str(time.strftime('%Y.%m.%d  %H.%M.%S'))+'.log')
log_file = open(filepath, 'w')
log_file.close()

#setup
logger = logging.getLogger('PyCMDS')
formatter = logging.Formatter(fmt='%(asctime)s.%(msecs).03d | CPU %(cpu)-2s RAM %(ram)-2s | %(levelname)-8s | Thread %(thread)-5s | %(origin)-20s | %(name)-20s | %(message)s',datefmt='%Y.%m.%d %H:%M:%S')

#set level
if g.debug.read(): logger.setLevel(logging.DEBUG) #default is info
...
class ContextFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        cpu_now = cpu.read()
        if cpu_now == '??': record.cpu = cpu_now
        else: record.cpu = str(int(cpu.read())).zfill(2)
...
def log(level, name, message = '', origin = 'name'):        
    ...
    #additional context
    if origin == 'name': origin = str(inspect.stack()[2][1]).split('\\')[-1].replace('.py', '')
