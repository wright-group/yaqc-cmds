...
class Path:
        ...
        if l:
            try:
                ...
                plt.text(
                    0,
                    -2,
                    "Max Overhead percentage: "
                    + str(round(100 * (l) / (0.5 * len(path) + l), 1))
                    + "%",
                )
            except TypeError:
    ...
    def calc_length(self, path=None):
        ...
        for i in range(len(path) - 1):
            ...
            for h in range(len(self.hws)):
                times_f.append(
                    self.time_cost(
                        h, self.grid[tuple(path[i])][h], self.grid[tuple(path[i + 1])][h]
                    )
                )
                times_b.append(
                    self.time_cost(
                        h, self.grid[tuple(path[-i - 1])][h], self.grid[tuple(path[-i - 2])][h]
                    )
                )
    ...
    def time_cost(self, hw_idx, start, stop):
        ...
        # motor_cost = 0
        moter_speed = lambda dist: 10616.2 - 10151 * np.exp(-0.00015155 * dist)
        if round(start, 2) == round(stop, 2):
            ...
            if dist < 0:
                dist = -dist
                rate = lambda s: -0.00005 * s * s + 0.0445 * s + 114.13
                if max(start, stop) > 1500:
                    return max(dist / (rate(stop / 10) * 10), 0.04)
                else:
                    return max(dist / (rate(stop)), 0.04)
            else:
                return mono_cost
        else:
...
class Snake:
    def __init__(self, input_grid, hardwares, units, start_point=None):
        ...
        try:
            assert len(self.grid.shape) == 3
        except:
            pass  # Figure out how to handle more than 2D

        if any(start_point):
            try:
                assert len(start_point) == len(units)
            except:
                pass  # Figure out how to encorporate starting/current position
