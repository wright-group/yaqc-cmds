...
class GUI(acquisition.GUI):
    ...
    def save(self, aqn_path):
        ...
        aqn.write('info', 'description', 'HOME: {} {}'.format(self.opa_combo.read(), homed_motor_names))
        # shared settings
        aqn.add_section('home')
        aqn.write('home', 'opa name', self.opa_combo.read())
        aqn.write('home', 'motor names', [motor.name for motor in self.opa_guis[self.opa_combo.read_index()].motors])
        # motor settings
        for motor in opa.motors:
