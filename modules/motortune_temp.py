...
class scan(QtCore.QObject):
    ...
    def run(self, inputs):
        ...
        grating_destinations = np.linspace(15, 29.5, 5)
        #bbo_destinations = np.linspace()        
        bbo_points = 15
        bbo_half_width = .2
        bbo_equ = lambda gra: np.interp(gra,a0,a1)

        npts = len(spec_destinations)*bbo_points#*len(grating_destinations)

        # initialize scan in daq
        daq.control.initialize_scan(daq_widget, module_name, scan_axes=['w2_grating','w2_bbo'], fit=False)
