...
D1 = dealys.hardwares[0]
D2 = dealys.hardwares[1]
...
# control objects
start = pc.Number(initial_value=0)
stop = pc.Number(initial_value=0)
...
class scan(QtCore.QObject):
    ...
    def run(self, inputs):
        ...
        destinations = np.linspace(start.read(), stop_color.read(), npts.read())
        ...
        daq.control.index_slice(col=a)
        
        hws = [MircoHR,OPA1,OPA2,OPA3,D1,D2]        
        ...
        for i in range(len(destinations)):
            if a<=5:
                ...
                hws[opa_index].set_moters(m_array)
        ...
        g.logger.log('info', '1D Scan done', '1D scan of '+ axis.read()+' complete)
        self.update_ui.emit()
        self.done.emit()
