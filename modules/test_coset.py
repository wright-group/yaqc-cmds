...
# control objects
start_color = pc.Number(initial_value=1500, units='wn')
stop_color = pc.Number(initial_value=2000, units='wn')
npts = pc.Number(initial_value=11, decimals=0)
...
class scan(QtCore.QObject):
    ...
    def run(self, inputs):
        ...
        daq_widget = inputs[1]
        gui = inputs[2]
        destinations = np.linspace(start_color.read('wn'), stop_color.read('wn'), npts.read())
        ...
        g.logger.log('info', 'test_coset_scan begun', '')

        # scan ----------------------------------------------------------------

        # initialize scan in daq
        daq.control.initialize_scan(daq_widget, scan_origin=module_name, scan_axes=['w1'], fit=False)
        daq.gui.set_slice_xlim(start_color.read('wn'), stop_color.read('wn'))
        ...
        for i in range(len(destinations)):
            OPA1.set_position(destinations[i], 'wn')
