...
from opas.pico.ps_curve_handler import Curve
...
class OPA:
    ...
    def initialize(self, inputs, address):
        ...
        if self.index >= 1 and self.index <= max_OPA_index:
            ...
            self.curve_path = ini.read('OPA{}'.format(self.index), 'curve path')
        else:
            error = True
            print('This is not a valid OPA number in pico_OPA.py.')
            g.logger.log('error','OPA'+str(self.index)+' does not exist.')

        if error:
            print('OPA'+str(self.index)+' initialization failed')
            g.logger.log('error','OPA inititailization failed')
